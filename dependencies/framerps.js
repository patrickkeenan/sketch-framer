var loadViews = function() {
	...
	createView = function(info, superView) {
		...
		console.log('setting extra frame att',info)
		view.rotationZ = info.layerFrame.rotationZ;
		view.opacity = info.layerFrame.opacity;

		if(info.styles){
			var stylestemp = info.styles.split(';');
			for (var x = 0, l = stylestemp.length-1; x < l; x++) {
				var values = stylestemp[x].split(':');
				console.log(values)
				if(values.length>1){
					view.style[values[0].trim()] = values[1].trim();
				}
